...
impl ShardTransactionTx {
    ...
    pub fn decode(tx: &[u8]) -> Result<UnverifiedTransaction, DecoderError> {
        ...
        for i in 0..gasl_rlp.item_count()? {
            let datas = gasl_rlp.at(i)?;

            // check if there is list of 2 items
            if datas.item_count()? != 2 {
                return Err(DecoderError::Custom("Unknown gas list length"));
            }
        ...
        Ok(UnverifiedTransaction::new(
            TypedTransaction::ShardTransaction(ShardTransactionTx {
                ...
                input_block_number: input_block_number_val,
    ...
    fn encode_payload(
        ...
        // gas list
        stream.begin_list(self.gas_list.len());
        for stuff in self.gas_list.iter() {
            stream.begin_list(2);
            stream.append(&stuff.0);
            stream.append(&stuff.1);
        }
    ...
    // encode by this payload spec: 0x03 | rlp([3, [chainId, nonce, maxFeePerGas(gasPrice), gasLimit, to, value, data, shard, next_shard, incomplete, hop_count, input_block_number, original_sender, shard_data_list, shard_proof_list, shard_proof, senderV, senderR, senderS]])
    pub fn encode(
        &self,
        chain_id: Option<u64>,
        signature: Option<&SignatureComponents>,
...
impl TypedTransaction {
    ...
    pub fn gas_list_replace_with(&mut self, v: Vec<(Address, U256)>){
        match self {
            // #[cfg(feature = "shard")]
            Self::ShardTransaction( tx) => {tx.gas_list = v},
            _ => {},
        }
    }
    ...
    pub fn push_gas_list(&mut self, a:Address, g:U256){
        match self {
            // #[cfg(feature = "shard")]
            Self::ShardTransaction( tx) => {tx.gas_list.push((a, g));},
            _ => {},
        }
    }
...
impl SignedTransaction {
    ...
    pub fn with_balance(mut self, balance: U256) -> SignedTransaction{
        let original_sender = self.original_sender().clone();
        self.transaction.unsigned = self.transaction.unsigned.with_balance(original_sender,balance);
        self
    }
    // #[cfg(feature = "shard")]
    ...
    // #[cfg(feature = "shard")]
    pub fn push_gas_list(&mut self, a: Address, g: U256){
        self.transaction.unsigned.push_gas_list(a,g);
    }
    pub fn gas_list(&self)->Vec<ShardProofItem> {
        match &self.transaction.unsigned {
            TypedTransaction::ShardTransaction(tx) => tx.gas_list.clone(),
            _ => Vec::new(),
        }
    }
    pub fn gas_list_replace_with(&mut self, v: Vec<(Address,U256)>){
        self.transaction.unsigned.gas_list_replace_with(v);
    }
    pub fn set_next_shard(&mut self, shard:u64){
