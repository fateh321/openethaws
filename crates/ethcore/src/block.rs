...
impl<'x> OpenBlock<'x> {
    ...
    pub fn push_transaction(
        ...
        if !data.is_empty() && self.block.state.get_mined_status()!=Some(true) {
            ...
            for datum in data{
                debug!(target:"txn","address looks like{} balance looks like {} shard looks like {}",datum.0.to_low_u64_be().rem_euclid(2u64.pow(AggProof::hyperproof_bits())),datum.1, t.shard_id());
                AggProof::pushAddressBalanceVerify(datum.0.to_low_u64_be().rem_euclid(2u64.pow(AggProof::hyperproof_bits())),datum.1.to_string(),t.shard_id());
            }
            let proof_result = AggProof::verifyProof(t.shard_proof(),t.shard_id(),0u64);
            debug!(target: "txn","verification is {:?}", proof_result);
        ...
        // we clear the load and store gas
        self.block.state.clear_load_store_gas();
        ...
        match self.block.state.get_mined_status(){
            ...
            debug!(target: "txn", "incomplete txn in enact, do nothing");
                ...
                if t.get_next_shard() == AggProof::get_shard(){
                    let my_id = AggProof::get_id();
                    let block_author_id = AggProof::author_id(self.block.header.author().clone());
                    if my_id > block_author_id{
                        if my_id-block_author_id <= (AggProof::shard_count()-1){
                            self.block.state.push_incomplete_txn(t.clone());
                        }
                    }else {
                        if (my_id-block_author_id+AggProof::node_count()) <= (AggProof::shard_count()-1){
                            self.block.state.push_incomplete_txn(t.clone());
                        }
                    }
        ...
        let account_proof_len = self.block.state.prove_account(keccak(t.original_sender())).unwrap().0.len();
        // for l in proof.iter(){
        //     proof_len += l.len();
        // }
