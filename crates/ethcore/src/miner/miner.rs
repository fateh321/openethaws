...
impl Miner {
        ...
        C: BlockChain + CallContract + BlockProducer + Nonce + Sync,
    {
        ...
        let (mut open_block, original_work_hash, engine_txs) = {
                ...
                .get_pending_if(|b| b.header.parent_hash() == &best_hash)
            {
                ...
                None => {
                    ...
                    // only import buffer amount of txn, drop the rest
                    let mut inchain_txn = chain.export_incomplete_txn();
                    if inchain_txn.len() > (AggProof::incomplete_txn_buffer_length() as usize){
                        inchain_txn.truncate(AggProof::incomplete_txn_buffer_length() as usize);
                    }
                    // block.set_incomplete_txn(chain.export_incomplete_txn());
                    block.set_incomplete_txn(inchain_txn);
                    ...
                    match self.engine.generate_engine_transactions(&block) {
                        Ok( transactions) => { let mut transactions = transactions;
                            let mut verify_transaction : Vec<SignedTransaction> = Vec::new();
            ...
            .chain(queue_txs.into_iter().map(|tx| tx.signed().clone()))
        {
            ...
            if (proof_data_count + 12u64) <= AggProof::block_data_count(){
                let current_number = block_number;
                if !transaction.is_incomplete(){
                    transaction.set_input_block_number(current_number);
                }
            }
