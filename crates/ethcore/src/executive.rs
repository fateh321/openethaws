        ...
        V: VMTracer,
    {
        ...
        let balance = match t.as_unsigned() {
            ...
            _ => {
                ...
                // println!("bal read legacy 1 tx looks like {:?}", t);
                self.state.balance(&sender)?},
        };
        ...
        let (result, output) = match t.tx().action {
            ...
            Action::Call(ref address) => {
                ...
                let mut res = self.call(params, &mut substate, &mut tracer, &mut vm_tracer);
                let load_store_gas = self.state.get_load_store_gas();
                let mut additional_gas = load_store_gas.0;
                additional_gas = additional_gas.saturating_add(load_store_gas.1);
                match &res{
                    Ok(res_inner) => {
            ...
            V: VMTracer,
    {
        ...
        let balance = match t.as_unsigned() {
            TypedTransaction::ShardTransaction(shard_tx) => match shard_tx.shard_data_list.get(&sender) {
                ...
                None => { AggProof::incr_bal_read_count(1u64);
                    println!("why the hell balance is not read??");
                    self.state.balance(&sender)?},
            },
            _ => {AggProof::incr_bal_read_count(1u64);
                println!("bal read legacy 2");
                self.state.balance(&sender)?},
        };
        // #[cfg(not(feature = "shard"))]
        // let balance = self.state.balance(&sender)?;
        ...
        let (result, output) = match t.tx().action {
            ...
            Action::Call(ref address) => {
                ...
                let mut res = self.call(params, &mut substate, &mut tracer, &mut vm_tracer);
                let load_store_gas = self.state.get_load_store_gas();
                let mut additional_gas = U256::zero();
                if self.state.txn_complete_status() == None {
                    additional_gas = load_store_gas.0;
                    if !self.state.is_reverted(){
                        additional_gas = additional_gas.saturating_add(load_store_gas.1);
                    }
