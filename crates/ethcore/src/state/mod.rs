...
pub struct State<B> {
    ...
    load_store_gas: RefCell<(U256,U256)>,
}
...
impl<B: Backend> State<B> {
    ...
    pub fn new(mut db: B, account_start_nonce: U256, factories: Factories) -> State<B> {
        ...
        State {
            ...
            load_store_gas: RefCell::new((U256::zero(), U256::zero())),
        }
    }
    ...
    pub fn clear_load_store_gas(&mut self) {self.load_store_gas = RefCell::new((U256::zero(), U256::zero()))}
    pub fn add_gas(&mut self, gas_0: u64, gas_1: u64) {
        let old_gas = self.load_store_gas.borrow().clone();
        let new_load_gas = old_gas.0.saturating_add(U256::from(gas_0));
        let new_store_gas = old_gas.1.saturating_add(U256::from(gas_1));
        self.load_store_gas = RefCell::new((new_load_gas, new_store_gas));
    }
    // pub fn add_store_gas(&mut self, gas: u64) {
    //     let old_gas = self.load_store_gas.borrow().clone();
    //     let new_store_gas = old_gas.1.saturating_add(U256::from(gas));
    //     self.load_store_gas = RefCell::new((new_store_gas, old_gas.1));
    // }
    pub fn get_load_store_gas(&self)->(U256, U256){
        self.load_store_gas.borrow().clone()
    }
...
impl Clone for State<StateDB> {
    fn clone(&self) -> State<StateDB> {
        ...
        State {
            ...
            load_store_gas: self.load_store_gas.clone(),
        }
    }
}
